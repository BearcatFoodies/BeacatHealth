...
public class PieRadarChartViewBase: ChartViewBase
{
    ...
    internal override func initialize()
    {
        ...
        _tapGestureRecognizer = NSUITapGestureRecognizer(target: self, action: "tapGestureRecognized")
        
        self.addGestureRecognizer(_tapGestureRecognizer)

        #if !os(tvOS)
            _rotationGestureRecognizer = NSUIRotationGestureRecognizer(target: self, action: "rotationGestureRecognized")
    ...
    internal final func processRotationGestureEnded(location location: CGPoint)
    {
        if isDragDecelerationEnabled
        {
            ...
            if _decelerationAngularVelocity != 0.0
            {
                ...
                _decelerationDisplayLink = NSUIDisplayLink(target: self, selector: "decelerationLoop")
